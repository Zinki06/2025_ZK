...
exports.thispost = async (req, res) => {
  ...
  const appliedTalents = await Promise.all(post.appliedTalents.map(async userId => {
  ...
  const matchedTalents = await Promise.all(post.matchedTalents.map(async userId => {
    const user = await User.findById(userId);
    return {
      userId,
      nickname: user.nickname,
...
exports.allposts = async (req, res) => {
  ...
  const posts = await Promise.all(allpost.map(async post => {
    return {
      postId: post._id,
